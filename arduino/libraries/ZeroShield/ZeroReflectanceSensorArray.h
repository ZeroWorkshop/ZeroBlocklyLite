...
#define ZeroReflectanceSensorArray_h
...
#include <Arduino.h>
  ...
  #define ZUMO_SENSOR_ARRAY_DEFAULT_EMITTER_PIN  255
#endif

class ZeroReflectanceSensorArray : public QTRSensorsRC
{
  public:
   ...
   */
  ZeroReflectanceSensorArray(unsigned char emitterPin)
  {
    init(emitterPin);
  }
   ...
   * settings as given.
   */
  ZeroReflectanceSensorArray(unsigned char * pins, unsigned char numSensors, unsigned int timeout = 2000,
   ...
   * \param emitterPin Pin that turns IR emitters on or off.
   *
   * This function initializes the ZeroReflectanceSensorArray object with the
   ...
   * boards). (The "LED ON" jumper on the Zero Reflectance Sensor Array must be
  ...
  {
    unsigned char sensorPins[] = {A0, A1, A4, A5, A2, A3 };
   ...
   * \param emitterPin Pin that turns IR emitters on or off.
   *
   * This function initializes the ZeroReflectanceSensorArray object with all
   * settings as given.
   *
   ...
   * the [product page](https://www.pololu.com/product/1419) for the Zero
   ...
   * jumper on the Zero Reflectance Sensor Array.) If the value
   ...
   *
   * ~~~{.ino}
   * ZeroReflectanceSensorArray reflectanceSensors;
   *
   * ...
   *
   ...
   * ~~~
   *
   * Alternatively, you can use \ref ZeroReflectanceSensorArray(unsigned char * pins, unsigned char numSensors, unsigned int timeout, unsigned char emitterPin)
   * "a different constructor" to declare and initialize the object at the same
   * time:
   *
   * ~~~{.ino}
   *
   * ZeroReflectanceSensorArray reflectanceSensors((unsigned char[]) {4, 5}, 2);
   * ~~~
   *
   */
...
\memberof ZeroReflectanceSensorArray
 ...
 * jumper or otherwise).
 *
 * The ZeroReflectanceSensorArray class inherits this function from the
 * QTRSensors class.
 */
 ...
 *
 * The ZeroReflectanceSensorArray class inherits this variable from the
 * QTRSensors class.
 *
