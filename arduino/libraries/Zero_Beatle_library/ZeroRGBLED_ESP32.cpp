#include "ZeroRGBLED_ESP32.h"



// constructor (doesn't do anything)

/*ZeroRGBLED_ESP32::ZeroRGBLED_ESP32(uint8_t pin, uint8_t num)
{
	  initialized = false;
		_pin = pin;
		_num = num;
...
void ZeroRGBLED_ESP32::colorAll(uint32_t c) {
	init();// 
  for(uint16_t i=0; i<_num; i++) {
...
void ZeroRGBLED_ESP32::colorWipe(uint32_t c, uint8_t wait,uint8_t num_of_pin) {
	init();// 
...
void ZeroRGBLED_ESP32::rainbow(uint8_t wait) {
	init();// 
  uint16_t i;
...
void ZeroRGBLED_ESP32::rainbowCycle(uint8_t wait) {
	init();// 
  uint16_t i, j;
...
void ZeroRGBLED_ESP32::theaterChase(uint32_t c, uint8_t wait) {
	init();// 
...
/*void ZeroRGBLED_ESP32::theaterChaseRainbow(uint8_t wait) {
	init();// 
...
uint32_t ZeroRGBLED_ESP32::Wheel(byte WheelPos) {
	init();// 
...
void ZeroRGBLED_ESP32::theaterChaseRainbow(int wait) {
	init();// 
...
void ZeroRGBLED_ESP32::rainbow_stream(uint8_t wait) {
	init();// 
...
void ZeroRGBLED_ESP32::FlashOneLED_RGB(int flash_speed, uint8_t pixel, uint32_t c) {
	init();// 
  long flash_time;
...
void ZeroRGBLED_ESP32::FlashOneLED_RGB(int flash_speed, uint8_t pixel, uint32_t c, int swap) {
	init();// 
  long flash_time;
...
void ZeroRGBLED_ESP32::OneLED_RGB(uint8_t pixel, uint32_t c) {
	init();// 
...
void ZeroRGBLED_ESP32::policeLightWipe(int pix_num, int *pixel) {
	init();// 
  if (firstRun) {
...
void ZeroRGBLED_ESP32::policeLightFlash(int pix_num, int flashSpeed, int swap) {
	init();// 
...
void ZeroRGBLED_ESP32::policeLightRun(int TotalPix, int *pixels) {
	init();// 
...
void ZeroRGBLED_ESP32::setBrightness(int value) {
	_brightness = value;
	strip.setBrightness(_brightness);
