...
bool BeatleSerial::begin(Stream &stream)
{
  _serial = &stream;
...
void BeatleSerial::progEEPROM(uint8_t prodY, uint8_t prodM, uint8_t prodD, uint8_t delY, uint8_t delM, uint8_t delD) {
...
void BeatleSerial::runWheel(uint8_t direct, uint8_t speed) {
...
void BeatleSerial::setLeftSpeed(uint8_t direct, uint8_t speed) {
  sendStack.commandValue  = beatleCMD::RUNLEFT;
  sendStack.feedbackValue = beatleCMD::NO_FEEDBACK;
  sendStack.paramMSB = direct;
  sendStack.paramLSB = speed;
...
void BeatleSerial::setLeftSpeed(int speeds) {
	uint8_t direct;
	uint8_t speed;
  if(speeds >0)
  {
  		direct = beatleCMD::RUNFORWARD;
  		speed = speeds;
  }
  else
  {
  		direct = beatleCMD::RUNBACKWARD;
  		speed = abs(speeds);
  }
  
  setLeftSpeed(direct,speed);
}


void BeatleSerial::setRightSpeed(uint8_t direct, uint8_t speed) {
  sendStack.commandValue  = beatleCMD::RUNRIGHT;
...
void BeatleSerial::setRightSpeed(int speeds) {
	uint8_t direct;
  uint8_t speed;
  if(speeds >0)
  {
  ...
  setRightSpeed(direct,speed);
}

void BeatleSerial::setSpeeds(int LeftSpeed, int RightSpeed) {
	uint8_t left_direct,left_speed;
	uint8_t right_direct,right_speed;
	if(LeftSpeed >0)
	{
		left_direct = beatleCMD::RUNFORWARD;
		left_speed = LeftSpeed;
	}
	else
	{
		left_direct = beatleCMD::RUNBACKWARD;
		left_speed = abs(LeftSpeed);		
	}
	if(RightSpeed >0)
	{
		right_direct = beatleCMD::RUNFORWARD;
		right_speed = RightSpeed;
	}
	else
	{
		right_direct = beatleCMD::RUNBACKWARD;
		right_speed = abs(RightSpeed);		
	}
	
	setLeftSpeed(left_direct, left_speed);
	setRightSpeed(right_direct, right_speed);	
}

void BeatleSerial::playBuzzer() {
...
void BeatleSerial::setLED(uint8_t ID, uint8_t value) {
  sendStack.commandValue  = beatleCMD::SETLED;
  sendStack.feedbackValue = beatleCMD::NO_FEEDBACK;

  sendStack.paramMSB = ID;
  sendStack.paramLSB = value;
...
void BeatleSerial::sendMazeGotoTargetCMD(const char* command_str) {
	...
	if (command_part2_count == 0)  {
		...
		for (i = 0; i < command_part1_count - 1; i++) {  
			sendStack.commandValue  = beatleCMD::SEND_MZSTR_P1;
      sendStack.feedbackValue = beatleCMD::NO_FEEDBACK;
      ...
      delay(10);
		...
		//2nd part
		{
			sendStack.commandValue  = beatleCMD::SEND_MZSTR_TP2;
      sendStack.feedbackValue = beatleCMD::NO_FEEDBACK;
...
void BeatleSerial::sendMazeGotoFinishCMD(const char* command_str) {
	uint8_t i;
	uint8_t cmdStr_length = strlen(command_str);
	...
	if (command_part2_count == 0)  {
      ...
      delay(10);
		...
		//2nd part
		{
			sendStack.commandValue  = beatleCMD::SEND_MZSTR_FP2;
      sendStack.feedbackValue = beatleCMD::NO_FEEDBACK;
...
int16_t BeatleSerial::buttonState(uint8_t button) {
...
int16_t BeatleSerial::frontIRSensors() {
...
int16_t BeatleSerial::lineSensors() {
  return query(beatleCMD::GET_LINESENSORS);
}


/////////////////////////////////////////////////////////
void BeatleSerial::findChecksum(stack *_stack)
{
...
int16_t BeatleSerial::query(uint8_t cmd, uint8_t msb, uint8_t lsb)
{
  flush();
...
bool BeatleSerial::getStatus(uint8_t cmd)
{
  timer = millis();
...
bool BeatleSerial::parseFeedback()
{                  
...
void BeatleSerial::printStack(stack _stack)
{
