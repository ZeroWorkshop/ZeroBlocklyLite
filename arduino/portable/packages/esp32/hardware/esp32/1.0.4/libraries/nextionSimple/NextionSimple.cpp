...
NextionSimple::NextionSimple(SoftwareSerial &next, uint32_t baud): nextion(&next){
  nextion->begin(baud);
  flushSerial();
}
#else
NextionSimple::NextionSimple(HardwareSerial &next, uint32_t baud): nextion(&next){
  nextion->begin(baud);
  flushSerial();
}
#endif

void NextionSimple::buttonToggle(boolean &buttonState, String objName, uint8_t picDefualtId, uint8_t picSelected){
  String tempStr = "";
  if (buttonState) {
...
uint8_t NextionSimple::buttonOnOff(String find_component, String unknown_component, uint8_t pin, int btn_prev_state){  
...
boolean NextionSimple::setComponentValue(String component, int value){
...
boolean NextionSimple::ack(void){
  /* CODE+END*/
  uint8_t bytes[4] = {0};
...
unsigned int NextionSimple::getComponentValue(String component){
...
boolean NextionSimple::setComponentText(String component, String txt){
  String componentText = component + ".txt=\"" + txt + "\"";//Set Component text
  sendCommand(componentText.c_str());
  return ack();
}//end set_component_txt

boolean NextionSimple::updateProgressBar(int x, int y, int maxWidth, int maxHeight, int value, int emptyPictureID, int fullPictureID, int orientation){
	int w1 = 0;
	int h1 = 0;
	int w2 = 0;
	int h2 = 0;
...
String NextionSimple::getComponentText(String component, uint32_t timeout){
...
String NextionSimple::listen(unsigned long timeout){//returns generic
...
uint8_t NextionSimple::pageId(void){
  sendCommand("sendme");
  int a = -1;
  String pagId = listen();
  //  Serial.print("ID = ");
...
void NextionSimple::sendCommand(const char* cmd){
...
boolean NextionSimple::init(const char* pageId){
...
void NextionSimple::flushSerial(){
  Serial.flush();
  nextion->flush();
}//end flush
