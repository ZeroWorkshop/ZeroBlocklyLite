...
Blockly.Field.prototype.updateTextNode_ = function() {
	if(this.textElement_) {
		var a = this.text_;
		a.length > this.maxDisplayLength && (a = a.substring(0, this.maxDisplayLength+50) );
...
Blockly.Xml.blockToDom2 = function(a) {
//	console.log(a.inputList[0].fieldRow[20].state_);
//	a.inputList[0].fieldRow[20].state_ = 'true';
	var arrTF=[];
...
//		console.warn("Deprecated call to Blockly.Xml.domToWorkspace, swap the arguments.")
	...
	for(var f = 0; f < c; f++) {
		...
		if("block" == h || "shadow" == h) {
			var h = Blockly.Xml.domToBlock(g, b),//here 
...
/**
 * Class for a mutator dialog.弹出框9131034
...
Blockly.Mutator.prototype.drawIcon_ = function(a) {
	Blockly.createSvgElement("rect", {//设置图标
		"class": "blocklyIconShape",
		rx: "4",
		ry: "4",
		height: "16",
...
Blockly.Mutator.prototype.createEditor_ = function() {//创建弹出窗口
...
Blockly.Mutator.prototype.resizeBubble_ = function() {//重新计算弹出框的大小
	var a = 2 * Blockly.Bubble.BORDER_WIDTH,
		b = this.workspace_.getCanvas().getBBox(),
		
		c;
	c = this.block_.RTL ? -b.x : b.width + b.x;//弹出框 宽度
	b = b.height + 3 * a	;//重新获取cb的长度
...
Blockly.Mutator.prototype.setVisible = function(a) {//设置可视化
	if(a != this.isVisible())//a为true的时候 =>显示mutator 反之，隐藏mutator
...
//			20 > d.y + e.height && c.moveBy(0, 20 - e.height - d.y)
	...
	if(this.rootBlock_.workspace == this.workspace_) {// Blockly.workspacesvg
		Blockly.Events.setGroup(!0);
		c = this.block_;//block父块
		a = (a = c.mutationToDom()) && Blockly.Xml.domToText(a);
		
		b = c.rendered;
		c.rendered = !1;
		if(e != undefined){
			c.compose(this.rootBlock_,e.height);
		}else{
			c.compose(this.rootBlock_);
		}
		
		c.rendered = b;//c.rendered=b=true
		c.initSvg();
		b = (b = c.mutationToDom()) && Blockly.Xml.domToText(b);//<mutation xmlns="http://www.w3.org/1999/xhtml" elseif="1"></mutation>
		if(a != b) {
...
Blockly.Mutator.prototype.getFlyoutMetrics_ = function() {//返回mutator的高度和宽度
	return {
...
Blockly.BlockSvg = function(a, b, c) {//9141519
	this.svgGroup_ = Blockly.createSvgElement("g", {}, null);//创建g元素
	this.svgPathDark_ = Blockly.createSvgElement("path", {//创建path元素 添加到g元素中 为path元素添加class和transform属性
...
Blockly.BlockSvg.prototype.initSvg = function() {//初始化svg
...
//		var j = Blockly.selected.inputList[2].fieldRow[19].class_.split('t')[0];
////		console.log('点击其他按钮时刻的选择数据'+j+'：'+Blockly.Xml.blockToDom2(Blockly.selected));
//		sessionStorage.setItem(j, JSON.stringify(Blockly.Xml.blockToDom2(Blockly.selected)));
//		setAgain();
//	}
...
Blockly.BlockSvg.render = {};//9181630
...
//	if(sessionStorage.getItem('donnt')==null||sessionStorage.getItem('donnt') == '1'){
//		sessionStorage.setItem('donnt','0');
//		return false;
//	}
//	
//	if(timeTurn%2==1){
...
//	}
//	if(Math.abs(nextTime - thisTime)<10){
//		return false;
//	}
...
////					console.log('点击其他按钮时刻的选择数据'+j+'：'+Blockly.Xml.blockToDom2(Blockly.selected));
//					sessionStorage.setItem(j, JSON.stringify(Blockly.Xml.blockToDom2(Blockly.selected)));
////					console.log(Blockly.Xml.blockToDom2(Blockly.selected))
//					setAgain();
//				}
//		}
...
Blockly.Generator.prototype.statementToCode = function(a, b) {//this, 'DO' + n	
...
Blockly.Css.inject = function(a, b) {
	if(!Blockly.Css.styleSheet_) {
		...
		Blockly.Css.mediaPath_ = b.replace(/[\\\/]$/, "");//获取media路径
//		console.log(c) c为css样式集
		var c = c.replace(/<<<PATH>>>/g, Blockly.Css.mediaPath_),//替换正确路径
			d = document.createElement("style");
		document.head.appendChild(d);
		c = document.createTextNode(c);//将c中的样式添加到html的head上
		d.appendChild(c);
		Blockly.Css.styleSheet_ = d.sheet;
//		console.log(Blockly.Css.Cursor.OPEN） handopen
		Blockly.Css.setCursor(Blockly.Css.Cursor.OPEN)
	}
};
...
Blockly.Css.CONTENT = [".blocklySvg {", "background-color: #fff;", "outline: none;", "overflow: hidden;", "}", ".blocklyWidgetDiv {", "display: none;", "position: absolute;", "z-index: 999;", "}", ".blocklyTooltipDiv {", "background-color: #ffffc7;", "border: 1px solid #ddc;", "box-shadow: 4px 4px 20px 1px rgba(0,0,0,.15);", "color: #000;", "display: none;", "font-family: sans-serif;", "font-size: 9pt;", "opacity: 0.9;", "padding: 2px;", "position: absolute;", "z-index: 1000;", "}", ".blocklyResizeSE {", "cursor: se-resize;",
	...
	"}", ".blocklyIconGroup:not(:hover),", ".blocklyIconGroupReadonly {", "opacity: 1;", "}", ".blocklyIconShape {", "fill: #535152;", "stroke: #fff;", "stroke-width: 1px;", "}", ".blocklyIconSymbol {", "fill: #fff;", "}", ".blocklyMinimalBody {", "margin: 0;", "padding: 0;", "}", ".blocklyCommentTextarea {", "background-color: #ffc;", "border: 0;", "margin: 0;", "padding: 2px;", "resize: none;", "}", ".blocklyHtmlInput {", "border: none;", "border-radius: 4px;", "font-family: sans-serif;", "height: 100%;", "margin: 0;", "outline: none;",
	...
	"font-size: 16px;", "padding: 0 3px;", "vertical-align: top;", "}", ".blocklyTreeSelected .blocklyTreeLabel {", "color: #fff;", "}", ".blocklyWidgetDiv .goog-palette {", "outline: none;", "cursor: default;", "}", ".blocklyWidgetDiv .goog-palette-table {", "border: 1px solid #666;", "border-collapse: collapse;", "}", ".blocklyWidgetDiv .goog-palette-cell {", "height: 13px;", "width: 15px;", "margin: 0;", "border: 0;", "text-align: center;", "vertical-align: middle;", "border-right: 1px solid #666;", "font-size: 1px;",
...
Blockly.createSvgElement = function(a, b, c, d) {//9181004
	a = document.createElementNS(Blockly.SVG_NS, a);//a为元素名
	for(var e in b) a.setAttribute(e, b[e]);//b为具有属性对象
	document.body.runtimeStyle && (a.runtimeStyle = a.currentStyle = a.style);//如果存在runtimestyle（ie下）则统一3种样式
	c && c.appendChild(a);//把a元素添加到c上
	return a
};
