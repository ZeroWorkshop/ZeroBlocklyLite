...
#define DIR_R  13
*/
ZeroMotors motors(PWM_L,DIR_L,PWM_R,DIR_R,BOARD_VERSION);
//ZeroMotors motors();
...
void turn(char dir)
{
  ...
  switch(dir)
  {
    case 'L':
    case 'B':
      motors.setSpeeds(-TURN_SPEED,TURN_SPEED,BOARD_VERSION);
      while(count<2)
      {
 ...
 void followSegment()
 {
    ...
    while(1)
    {
        ...
        if(power_difference < 0)
            motors.setSpeeds(SPEED + power_difference, SPEED,BOARD_VERSION);
        else
            motors.setSpeeds(SPEED, SPEED - power_difference,BOARD_VERSION);
...
void solveMaze()
{
 ...
 // if(ABOVE_LINE(sensors[0]) || ABOVE_LINE(sensors[1]) )
  if(ABOVE_LINE(sensors[0]) )
    ...
    found_right = 1;
    motors.setSpeeds(SPEED,SPEED,BOARD_VERSION);
    delay(_OVERSHOOOT_);
    getFinalData(0x9,sensors);
    if(ABOVE_LINE(sensors[0])  )
    found_left = 1;
   ...
   getFinalData(0x9,sensors);
    ...
    if(ABOVE_LINE(sensors[1]) ||ABOVE_LINE(sensors[2]) && ABOVE_LINE(sensors[3]) && ABOVE_LINE(sensors[4]) && ABOVE_LINE(sensors[5])|| ABOVE_LINE(sensors[6]))
        {
          motors.setSpeeds(0,0,BOARD_VERSION);
          break;
        }
...
void setup()
{
  ...
  for(int i = 0; i < 160; i++)
  {
    if ((i > 20 && i <= 60) || (i > 100 && i <= 140))
      motors.setSpeeds(-200, 200,BOARD_VERSION);
    else
      motors.setSpeeds(200, -200,BOARD_VERSION);
      calibration();
    delay(8);
